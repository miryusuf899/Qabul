      ...
      <div className="mx-auto flex max-w-[1480px] items-center gap-3">
        <div className="hidden flex-1 items-center rounded-full bg-white px-3 py-2 ring-1 ring-qabul-ink/[0.08] md:flex">
          ...
          <input
            ...
            placeholder="Поиск по клиентам, услугам и записям"
            type="search"
          />
        </div>
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <div className="relative">
            ...
            >
              {businesses.length === 0 ? <option value="">Нет бизнеса</option> : null}
          ...
          <button className="btn btn-secondary size-10 p-0" aria-label="Уведомления">
            <Bell size={17} strokeWidth={1.8} />
          </button>
          <div className="hidden min-w-0 flex-col px-2 text-right sm:flex">
            <span className="truncate text-sm font-semibold">{user?.full_name || 'Владелец'}</span>
            <span className="truncate text-xs text-qabul-muted">{user?.email}</span>
          </div>
          <button className="btn btn-secondary size-10 p-0" onClick={logout} aria-label="Выйти">
